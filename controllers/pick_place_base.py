...
class PickPlaceControllerBase(BaseController):
    """
    ...
    def __init__(
        ...
        operation_mode: str = 'pick_place',  # 新增参数
    ) -> None:
        ...
        if self._operation_mode == 'pick':
            self._start_event = 0
            self._end_event = 4
        elif self._operation_mode == 'place':
            self._start_event = 5
            self._end_event = 9
        else:
            self._start_event = 0
    ...
    def forward(
        ...
        initial_end_effector_position: typing.Optional[np.ndarray] = None, 
    ) -> ArticulationAction:
        ...
        if self._event == self._start_event:
          if self._operation_mode == 'pick':
              self._current_target_x = picking_position[0]
              self._current_target_y = picking_position[1]
              self._h0 = picking_position[2]
          elif self._operation_mode == 'place':
              if initial_end_effector_position is None:
                  raise ValueError("In 'place' mode, 'initial_end_effector_position' must be provided.")
              self._current_target_x = initial_end_effector_position[0]
              self._current_target_y = initial_end_effector_position[1]
              self._h0 = initial_end_effector_position[2]
          else:
              self._current_target_x = picking_position[0]
              self._current_target_y = picking_position[1]
              self._h0 = picking_position[2]
        ...
        if self._event == 2:
            ...
            if self._event in [0, 1]:
                ...
                self._current_target_y = picking_position[1]
                self._h0 = picking_position[2]
            if self._operation_mode == 'pick' or placing_position is None:
                # If placing_position is None, use current target position for interpolation
                interpolated_xy = np.array([self._current_target_x, self._current_target_y])
                target_height = self._get_target_hs(self._h0)
            else:
    ...
    def is_done(self) -> bool:
        ...
        """
        if self._event > self._end_event:
            return True
        else:
            return False
